            ...
            [
                {
                    ...
                    "activating": ex.get("activating", False),
                    "prediction": ex.get("prediction", False),
                    "probability": ex.get("probability", None),
...
def frequency_weighted_f1(
    ...
    for (module, latent_idx), grp in df.groupby(["module", "latent_idx"]):
        ...
        # Handle case where latent_idx might not exist in counts
    ...
    for module in latent_df["module"].unique():
        ...
        # Handle case where latent indices might not exist in counts tensor
        valid_indices = []
        firing_weights = []
        for idx in module_df["latent_idx"]:
            if idx < len(counts[module]):
                valid_indices.append(idx)
                firing_weights.append(counts[module][idx].item())
            else:
                # Skip features that don't exist in counts
                continue
        
        if not firing_weights:
            continue
            
        firing_weights = torch.tensor(firing_weights, dtype=torch.float32)
        total_weight = firing_weights.sum()
        if total_weight == 0:
            continue

        # Only use F1 scores for valid indices
        valid_f1_scores = module_df[module_df["latent_idx"].isin(valid_indices)]["f1_score"].values
        f1_tensor = torch.as_tensor(valid_f1_scores, dtype=torch.float32)
        module_f1 = (f1_tensor * firing_weights).sum() / firing_weights.sum()
        per_module_f1.append(module_f1)

    if not per_module_f1:
        return None
