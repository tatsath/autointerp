...
def load_tokenized_data(
    ...
    # Preprocess OpenThoughts-114k conversations to text if needed
    if column_name == "text" and "text" not in data.column_names:
        if "conversations" in data.column_names:
            def extract_text_from_conversations(example):
                """Extract text from OpenThoughts-114k conversations format."""
                text_parts = []
