...
A training-free approach to identify sparse features aligned to reasoning, finance, healthcare, science, math, tool-calling, or other domain concepts using activation-separation-based scoring.
...
```
feature_search/
├── main/                      # Core functionality (REQUIRED)
│   ├── run_feature_search.py  # Main entry point - use this directly
...
│
├── domains/                   # OPTIONAL: Convenience wrappers (can be removed)
│   ├── finance/               # Example: Wrapper script + config.json
│   └── _template/             # Template for wrappers (not needed)
│
