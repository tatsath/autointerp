...
def compute_score(
    ...
    if torch.cuda.is_available():
        try:
            # Try to allocate a small tensor to check if there's free memory
            test_tensor = torch.zeros(1, device="cuda")
            del test_tensor
            torch.cuda.empty_cache()
            device = "cuda"
        except RuntimeError:
            # CUDA OOM or other error, fallback to CPU
            device = "cpu"
            print(">>> Warning: CUDA unavailable or out of memory, using CPU")
    ...
    if sae_id is None:
        ...
        # For local paths, manually load and convert weights
        ...
        if layer_match:
            ...
            if os.path.exists(layer_path):
                ...
                # Ensure k is in config for TopK (it might be checking activation_fn_kwargs)
