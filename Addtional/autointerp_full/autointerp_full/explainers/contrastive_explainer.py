...
from autointerp_full.explainers.default.prompts import SYSTEM_CONTRASTIVE
from autointerp_full.explainers.explainer import Explainer, ExplainerResult, Response
from autointerp_full.latents.latents import ActivatingExample, LatentRecord, NonActivatingExample
...
class ContrastiveExplainer(Explainer):
    ...
    async def __call__(self, record: LatentRecord) -> ExplainerResult:
        ...
        # Explicitly sort by max_activation to ensure highest-activation examples come first
        ...
        if len(record.not_active) > 0:
            ...
            for example in non_activating_examples:
                if not hasattr(example, 'normalized_activations') or example.normalized_activations is None:
    ...
    def _build_prompt(  # type: ignore
        ...
        if activating_examples:
            total_examples = len(activating_examples)
            highlighted_examples.append(f"ACTIVATING EXAMPLES (ordered by activation strength, highest first, showing {total_examples} examples):")
            highlighted_examples.append("IMPORTANT: Your explanation must represent patterns from AT LEAST 40-60% of these examples (top examples are more important).")
            for i, example in enumerate(activating_examples, 1):
                str_toks = example.str_tokens
                activations = example.activations.tolist()
                max_act = example.max_activation if hasattr(example, 'max_activation') else max(activations) if activations else 0.0
                # Calculate percentile (top examples are higher percentile)
                percentile = ((total_examples - i + 1) / total_examples) * 100
                ex = self._highlight(str_toks, activations).strip().replace("\n", "")
                highlighted_examples.append(f"Example {i} (max_activation={max_act:.2f}, top {percentile:.0f}%):  {ex}")
