...
def compute_domain_score(
    ...
    parameters from config.json/config.yaml and applies quantile filtering to select top features.
    """
    ...
    tokens_path = config.get('tokens_str_path', 'tokens.json')
    if not os.path.isabs(tokens_path):
        tokens_path = os.path.join(config_dir, tokens_path)
    
    # If tokens.txt, convert to JSON; if tokens.json, use it directly
    ...
    print(">>> Computing DomainScore using core compute_score function...")
    compute_score(
