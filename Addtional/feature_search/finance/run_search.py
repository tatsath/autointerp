#!/usr/bin/env python3
"""
Finance domain feature search script.
...
def load_config(config_path: str = None):
    ...
    # Resolve output_dir relative to feature_search root
    if 'output_dir' in config and not os.path.isabs(config['output_dir']):
        feature_search_root = os.path.dirname(script_dir)
        config['output_dir'] = os.path.join(feature_search_root, config['output_dir'])
        os.makedirs(config['output_dir'], exist_ok=True)
    
    return config


def run_finance_search(
    model_path: str = None,
    sae_path: str = None,
    sae_id: str = None,
    ...
    **kwargs
    ...
    """
    Run finance feature search with configuration from config.yaml.
    
    Args:
        model_path: Override model path from config
        sae_path: Override SAE path from config
        sae_id: Override SAE ID from config
        config_path: Path to config.yaml (default: finance/config.yaml)
    ...
    search_args = {
        ...
        'score_type': config.get('score_type', 'domain'),
