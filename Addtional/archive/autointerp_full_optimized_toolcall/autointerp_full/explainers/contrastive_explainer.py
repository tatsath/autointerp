...
try:
    from autointerp_full.explainers.default.prompts_semantic import SYSTEM_CONTRASTIVE
    PROMPT_SOURCE = "prompts_semantic"
    print(f"✅ Using prompt from: {PROMPT_SOURCE}")
except ImportError:
    try:
        from autointerp_full.explainers.default.prompts_toolcall import SYSTEM_CONTRASTIVE
        PROMPT_SOURCE = "prompts_toolcall"
        print(f"⚠️  Using prompt from: {PROMPT_SOURCE} (prompts_semantic not found)")
    except ImportError:
        from autointerp_full.explainers.default.prompts import SYSTEM_CONTRASTIVE
        PROMPT_SOURCE = "prompts (default)"
        print(f"⚠️  Using prompt from: {PROMPT_SOURCE} (prompts_semantic and prompts_toolcall not found)")
...
class ContrastiveExplainer(Explainer):
    ...
    def _build_prompt(  # type: ignore
        ...
        if self.verbose:
            from autointerp_full import logger
            logger.info(f"Using prompt from: {PROMPT_SOURCE}")
            logger.info(f"Prompt preview: {SYSTEM_CONTRASTIVE[:200]}...")
