...
from autointerp_full.config import CacheConfig
from autointerp_full.latents.collect_activations import collect_activations
...
class LatentCache:
    ...
    def __init__(
        ...
        # Wrap encoding functions to use selective encoding if feature_indices provided
        self.hookpoint_to_sparse_encode = {}
        for hookpoint, encode_fn in hookpoint_to_sparse_encode.items():
            if hookpoint in self.feature_indices:
                # Wrap with selective encoding
                from autointerp_full.sparse_coders.load_sparsify import sae_dense_latents_selective
                # Extract SAE from the partial function
                # The encode_fn is a partial(sae_dense_latents, sae=sae)
                if hasattr(encode_fn, 'func') and hasattr(encode_fn, 'keywords'):
                    sae = encode_fn.keywords.get('sae')
                    if sae is not None:
                        # Create new wrapper with selective encoding (fix closure issue)
                        feat_idx = self.feature_indices[hookpoint]
                        def make_selective_wrapper(sae_obj, feat_idx_tensor):
                            return lambda x: sae_dense_latents_selective(x, sae_obj, feat_idx_tensor)
                        self.hookpoint_to_sparse_encode[hookpoint] = make_selective_wrapper(sae, feat_idx)
                    else:
                        self.hookpoint_to_sparse_encode[hookpoint] = encode_fn
                else:
                    self.hookpoint_to_sparse_encode[hookpoint] = encode_fn
            else:
                self.hookpoint_to_sparse_encode[hookpoint] = encode_fn
