#!/usr/bin/env python3
"""
Generate enhanced CSV summary for AutoInterp tool-call feature results.
...
def generate_enhanced_csv(results_dir, output_name="toolcall_results_summary_enhanced.csv"):
    """Generate enhanced CSV file with results summary for tool-call features.

    Args:
        results_dir: Directory containing AutoInterp outputs.
        output_name: Filename for the enhanced CSV summary (defaults to toolcall_results_summary_enhanced.csv).
    """
    ...
    for explanation_file in explanations_dir.glob("*.txt"):
        ...
        # Format: backbone.layers.19_latent186.txt or layers.19_latent186.txt
        filename = explanation_file.stem
        
        if "latent" not in filename:
            continue
        
        # Parse backbone.layers.19_latent186 or layers.19_latent186 format
        parts = filename.split("_")
        if len(parts) < 2:
            continue
        
        layer_part = parts[0]  # backbone.layers.19 or layers.19
        feature_part = parts[1]  # latent186
        
        # Extract layer number from backbone.layers.19 or layers.19
        ...
        # Extract feature number from latent186
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate enhanced CSV summary for AutoInterp tool-call feature results"
    )
    parser.add_argument(
        "results_dir",
    ...
    parser.add_argument(
        "--output_name",
        default="toolcall_results_summary_enhanced.csv",
        help="Filename for the enhanced CSV summary (default: toolcall_results_summary_enhanced.csv)",
    )
