...
class CacheConfig(Serializable):
    ...
    Can be a HuggingFace Hub dataset name or a local file path (TXT, JSON, CSV).
    For multiple files, use comma-separated paths."""

    dataset_split: str = "train[:1%]"
    """Dataset split to use for generating latent activations (ignored for local files)."""

    dataset_name: str = ""
    """Dataset name to use (ignored for local files)."""

    dataset_column: str = "text"
    """Dataset row to use."""

    local_files: list[str] = list_field(default=[])
    """Optional list of local file paths (TXT, JSON, CSV) to combine with dataset_repo.
    These files will be loaded and concatenated together."""
...
class RunConfig(Serializable):
    ...
    models, 'openrouter' for API calls, and 'vllm' for vLLM server."""
    
    explainer_api_base_url: str | None = field(
        default=None,
    )
    """API base URL for explainer providers that use API calls (e.g., vLLM, OpenRouter)."""
    ...
    feature_num: list[int] | None = None
    """Specific feature numbers to explain. If provided, overrides max_latents."""
