...
from autointerp_full.utils import (
    load_tokenized_data,
    to_int64_tensor,
)
...
class LatentDataset:
    ...
    def load_tokens(self):
        ...
        if not hasattr(self, "tokens"):
            self.tokens = load_tokenized_data(
                ...
                local_files=self.cache_config.get("local_files", None),
            )
        return self.tokens
    ...
    def _build_selected(
        ...
        for module in self.modules:
            ...
            # Map each latent to the edge (file) that contains it
            # This handles non-contiguous file ranges correctly
            edge_to_latents = {}
            for latent in selected_latents:
                latent_val = latent.item() if isinstance(latent, torch.Tensor) else latent
                found = False
                for start, end in edges:
                    if start <= latent_val <= end:
                        if (start, end) not in edge_to_latents:
                            edge_to_latents[(start, end)] = []
                        edge_to_latents[(start, end)].append(latent)
                        found = True
                        break
                if not found:
                    raise ValueError(
                        f"Latent {latent_val} not found in any file for module {module}. "
                        f"Available file ranges: {edges}"
                    )
            
            # Create buffers for each edge that contains selected latents
            for (start, end), _selected_latents_list in edge_to_latents.items():
                # Convert list of tensors/scalars to a single tensor
                if isinstance(_selected_latents_list[0], torch.Tensor):
                    _selected_latents = torch.stack(_selected_latents_list) if len(_selected_latents_list) > 1 else _selected_latents_list[0]
                else:
                    _selected_latents = torch.tensor(_selected_latents_list)
