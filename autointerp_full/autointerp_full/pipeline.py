    ...
    async def run(self, max_concurrent: int = 10) -> list[Any]:
        ...
        async for item in self.generate_items():
            ...
            if len(tasks) >= max_concurrent:
                ...
                # Handle results with error catching to continue processing
                for task in done:
                    try:
                        result = task.result()
                        ...
                        logger.error(f"Task failed with error: {repr(e)}")
                        logger.warning("Continuing with remaining tasks...")
                tasks = pending
            ...
            for pipe in self.pipes:
                if result is not None:
                    try:
                        ...
                        from autointerp_full import logger
                        logger.error(f"Error processing item through pipe: {repr(e)}")
                        # Return the item with error info so pipeline can continue
                        # The pipe should handle errors internally, but this is a safety net
                        if hasattr(result, 'latent'):
                            logger.warning(f"Continuing despite error for {result.latent}")
                        result = None  # Skip remaining pipes for this item
                        break
                else:
                    pass
        return result
