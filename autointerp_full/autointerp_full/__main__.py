...
from autointerp_full.latents.neighbours import NeighbourCalculator
from autointerp_full.log.result_analysis import log_results
from autointerp_full.pipeline import Pipe, Pipeline, process_wrapper
from autointerp_full.scorers import DetectionScorer, FuzzingScorer, OpenAISimulator
from autointerp_full.sparse_coders import load_hooks_sparse_coders, load_sparse_coders
from autointerp_full.utils import assert_type, load_tokenized_data
...
async def process_cache(
    ...
    if not run_cfg.explainer == "none":
        ...
        if run_cfg.explainer == "np_max_act":
        ...
        elif run_cfg.constructor_cfg.non_activating_source == "FAISS":
            explainer = ContrastiveExplainer(
                llm_client,
                threshold=0.3,
                verbose=run_cfg.verbose,
                max_examples=50,  # Increased to 50 for more examples to find common patterns
                max_non_activating=15,  # Increased to 15 for better contrast
            )
        else:
