...
# AutoInterp Analysis - Llama-3.1-8B-Instruct 10 Features Test
...
# Load top 10 finance features from feature_list.json
FEATURE_LIST_FILE="/home/nvidia/Documents/Hariom/autointerp/search_autointerp/results/1_search/llama_3_1_8b_instruct_finance_l19_20251128_212146/feature_list.json"
if [ -f "$FEATURE_LIST_FILE" ]; then
    # Extract top 10 feature indices using Python
    FEATURE_LIST=$(python3 -c "
import json
with open('$FEATURE_LIST_FILE', 'r') as f:
    data = json.load(f)
    top10 = data['feature_indices'][:10]
    print(' '.join(map(str, top10)))
")
    echo "Loaded top 10 finance features: $FEATURE_LIST"
else
    echo "Warning: Feature list file not found, using default 0-9"
...
# FEATURE_LIST is now set above from JSON file or defaults to 0-9
...
# Remove old results to force regeneration for new features
if [ -d "results/$RUN_NAME" ]; then
    echo "Removing old results to regenerate for new features..."
