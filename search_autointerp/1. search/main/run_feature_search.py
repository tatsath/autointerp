...
# Dashboard moved to 3. autointerp_advance - import only if needed
try:
    import sys
    import os
    sys.path.insert(0, os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "3. autointerp_advance"))
...
def run_feature_search(
    ...
    # Note: Dashboard moved to 3. autointerp_advance - this is kept for backward compatibility
    if generate_dashboard:
        print("Step 3: Generating dashboard...")
        print("-" * 80)
        print("⚠️  Note: Dashboard generation has been moved to 3. autointerp_advance/")
        print("   Use '3. autointerp_advance/compute_dashboard.py' instead")
        print()
        
        if compute_dashboard is not None:
            try:
                compute_dashboard(
                    model_path=model_path,
                    sae_path=sae_path,
