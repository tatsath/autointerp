...
python "$BASE_DIR/1. search/domains/finance/compute_finance_score.py" \
    ...
    --config_path "$BASE_DIR/1. search/domains/finance/config.json" \
...
python "$BASE_DIR/../3. autointerp_advance/compute_dashboard.py" \
    --model_path meta-llama/Llama-3.1-8B-Instruct \
    --sae_path /home/nvidia/Documents/Hariom/saetrain/trained_models/llama3.1_8b_layer19_k32_latents400_lmsys_chat1m_multiGPU \
    --dataset_path jyanimaulik/yahoo_finance_stockmarket_news \
    --scores_dir "$BASE_DIR/1. search/domains/finance/scores" \
    --sae_id blocks.19.hook_resid_post \
    --num_features 200 \
    --n_samples 10000 \
    --minibatch_size_features 128 \
    --minibatch_size_tokens 64 \
    --output_dir "$BASE_DIR/1. search/domains/finance/dashboards"
...
echo "   • Scores: $BASE_DIR/1. search/domains/finance/scores/"
echo "   • Top features: $BASE_DIR/1. search/domains/finance/scores/top_features.pt"
echo "   • Dashboard: $BASE_DIR/1. search/domains/finance/dashboards/features-200.html"
echo ""
