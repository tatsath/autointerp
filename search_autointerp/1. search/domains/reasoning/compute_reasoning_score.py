...
# File is now in: 1. search/domains/reasoning/compute_reasoning_score.py
...
def compute_reasoning_score(
    ...
    sae_path: str,
    sae_id: str = None,
    config_path: str = "domains/reasoning/config.json",
    ...
    """Compute ReasoningScore using domain-specific configuration.
    
    This wrapper uses the core compute_score function with reasoning-specific
    parameters from config.json and applies quantile filtering to select top features.
    """
    # Load reasoning-specific configuration (resolve relative to script location)
    ...
    if output_dir is None:
        # Extract model name from model_path (e.g., "nvidia/NVIDIA-Nemotron-Nano-9B-v2" -> "nvidia_nemotron_nano_9b_v2")
        ...
        # Create output folder: {model_name}_reasoning_{layer}_{run_name}
        output_dir = os.path.join(base_output_dir, f"{model_name}_reasoning_{layer_suffix}_{run_name}")
    ...
    print(">>> Computing ReasoningScore using core compute_score function...")
    compute_score(
